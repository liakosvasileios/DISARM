...
int main() {
    ...
    uint8_t add_rax_imm[] = {0x48, 0x05, 0x78, 0x56, 0x34, 0x12}; // add rax, 0x12345678
    uint8_t sub_rax_imm[] = {0x48, 0x2D, 0x05, 0x00, 0x00, 0x00}; // sub rax, 5
    uint8_t mov_reg32_imm[] = {0xB8, 0x34, 0x12, 0x00, 0x00}; // mov eax, 0x1234
    ...
    test_mutation("add rax, 0x12345678 => MBA decomposition", add_rax_imm, sizeof(add_rax_imm));
    test_mutation("sub rax, 0x5 => MBA decomposition", sub_rax_imm, sizeof(sub_rax_imm));
    test_mutation("mov eax, 0x1234 => MBA decomposition", mov_reg32_imm, sizeof(mov_reg32_imm));
    test_mutation("jne near => SETcc + TEST + JNZ + JMP", jcc_near, sizeof(jcc_near));
